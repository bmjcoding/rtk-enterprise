...
enum Commands {
    ...
    /// Execute a shell command via sh -c (raw, no filtering)
    Run {
    ...
    /// Execute command without filtering
    Proxy {
...
const RTK_META_COMMANDS: &[&str] = &[
    "init", "config", "proxy", "run", "hook", "pipe", "verify", "trust", "untrust", "rewrite",
];

/// Previously built-in commands removed from the enterprise build. They must
/// not fall through to an arbitrary executable with the same name.
...
fn run_fallback(parse_error: clap::Error) -> Result<i32> {
    ...
    // e.g. `rtk init --badtypo` should show Clap's error, not try to run `init` from $PATH.
    if RTK_META_COMMANDS.contains(&args[0].as_str()) || REMOVED_COMMANDS.contains(&args[0].as_str())
    {
        parse_error.exit();
    }
    ...
    // Legacy no-op timing shim retained for command wrapper compatibility.
    let timer = core::execution::CommandExecution::start();
    ...
    if let Some(filter) = toml_match {
        ...
        match result {
            Ok(output) => {
                ...
                // Retained compatibility hook; enterprise builds do not persist raw output.
                ...
                core::execution::record_parse_failure_noop(&raw_command, &error_message, true);

                Ok(exit_code)
            }
            Err(e) => {
                // Command not found — same behaviour as no-TOML path
                core::execution::record_parse_failure_noop(&raw_command, &error_message, false);
                eprintln!("[rtk: {}]", e);
                Ok(127)
            }
        }
        ...
        match status {
            Ok(s) => {
                timer.finish_passthrough(&raw_command, &format!("rtk fallback: {}", raw_command));
...
fn run_cli() -> Result<i32> {
    ...
    // Meta commands (init, verify, config, etc.) skip the check
            ...
            match args[0].as_str() {
                ...
                "prisma" => {
                    ...
                    if args.len() > 1 {
                        ...
                        timer.finish_passthrough("npx prisma", "rtk npx prisma (passthrough)");
                        core::utils::exit_code_from_status(&status, "npx prisma")
                    }
                }
...
/// Meta commands (init, verify, config, etc.) are excluded because
...
mod tests {
    ...
    fn test_npx_unknown_tool_passthrough() {
        // Unknown tools under `rtk npx` must remain delegated to npx.
