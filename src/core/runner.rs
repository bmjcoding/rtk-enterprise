    ...
    match mode {
        RunMode::Filtered(filter_fn) => {
            ...
            if opts.skip_filter_on_failure && exit_code != 0 {
                ...
                timer.finish(&cmd_label, &format!("rtk {}", cmd_label), raw, raw);
                return Ok(exit_code);
            }
            ...
            let raw_for_output = if opts.filter_stdout_only {
                raw_stdout
            } else {
                raw
            };
        ...
        RunMode::Passthrough => {
            ...
            timer.finish_passthrough(&cmd_label, &format!("rtk {} (passthrough)", cmd_label));
            Ok(result.exit_code)
        }
    }
}
