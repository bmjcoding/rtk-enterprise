...
**Command execution and output filtering.** Every module here calls an external CLI tool (`Command::new("some_tool")`) and transforms its stdout/stderr to reduce token consumption.
...
Does **not** own: the TOML DSL filter engine (that's `core/toml_filter`) or hook interception (that's `hooks/`).
...
## Purpose
All command-specific filter modules that execute CLI commands and transform their output to minimize LLM token consumption. Each module follows a consistent pattern: execute the underlying command, filter its output through specialized parsers, and propagate exit codes.
...
The shared wrappers in [`core/runner.rs`](../core/runner.rs) encapsulate the execution skeleton. Modules build the `Command` (custom arg logic), then delegate to a runner entry point. All runners handle filtering and exit code propagation automatically.

```
 run_streaming()       Filter applied
...
| No filtering | `run_passthrough()` | Passthrough | Zero overhead, inherits TTY |
...
3. **Print** — filtered output is written to stdout (live for Streaming, post-hoc for CaptureOnly/Buffered)
4. **Exit code** — returns `Ok(exit_code)` to caller; `main.rs` calls `process::exit(code)` once
...
| `RunOptions::with_tee("label")` | Combined filtering; no persistence in enterprise builds |
| `RunOptions::stdout_only()` | Stdout-only to filter, stderr passthrough |
| `RunOptions::stdout_only().tee("label")` | Stdout-only filtering; no persistence in enterprise builds |
...
Exit code handling is **fully automatic** when using `run_filtered()` — the wrapper extracts the exit code (including Unix signal handling via 128+signal) and returns `Ok(exit_code)`. Module authors just return the result.
...
**Level 2: `BlockHandler` trait** — custom block detection with state transitions
...
Raw-output persistence is disabled in this enterprise build. Existing runner options that mention tee are compatibility no-ops.
...
1. **Create module** in `src/cmds/<ecosystem>/mycmd_cmd.rs`:
   ...
   - Avoid adding filesystem persistence for raw command output
