...
fn run_api(args: &[String], _verbose: u8) -> Result<i32> {
    ...
    // Passthrough preserves the full response without filtering.
...
mod tests {
    ...
    fn test_extract_identifier_with_repo_flag_after() {
        // gh issue view 185 -R owner/repo
        let args: Vec<String> = vec!["185".into(), "-R".into(), "owner/repo".into()];
        let (id, extra) = extract_identifier_and_extra_args(&args).unwrap();
        assert_eq!(id, "185");
    ...
    fn test_extract_identifier_with_repo_flag_before() {
        // gh issue view -R owner/repo 185
        let args: Vec<String> = vec!["-R".into(), "owner/repo".into(), "185".into()];
        let (id, extra) = extract_identifier_and_extra_args(&args).unwrap();
        assert_eq!(id, "185");
        assert_eq!(extra, vec!["-R", "owner/repo"]);
    }
