...
fn run_generate(args: &[String], verbose: u8) -> Result<i32> {
    ...
    if !result.success() {
        ...
        timer.finish("prisma generate", "rtk prisma generate", &raw, &raw);
        return Ok(result.exit_code);
    }

    let filtered = filter_prisma_generate(&raw);
    println!("{}", filtered);
    timer.finish("prisma generate", "rtk prisma generate", &raw, &filtered);
...
fn run_migrate(subcommand: MigrateSubcommand, args: &[String], verbose: u8) -> Result<i32> {
    ...
    if !result.success() {
        ...
        timer.finish(cmd_name, &format!("rtk {}", cmd_name), &raw, &raw);
        return Ok(result.exit_code);
    }
    ...
    println!("{}", filtered);
    timer.finish(cmd_name, &format!("rtk {}", cmd_name), &raw, &filtered);
...
fn run_db_push(args: &[String], verbose: u8) -> Result<i32> {
    ...
    if !result.success() {
        ...
        timer.finish("prisma db push", "rtk prisma db push", &raw, &raw);
        return Ok(result.exit_code);
    }

    let filtered = filter_db_push(&raw);
    println!("{}", filtered);
    timer.finish("prisma db push", "rtk prisma db push", &raw, &filtered);
