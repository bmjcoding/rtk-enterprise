...
// - On success: raw filtering input includes stderr (previously stdout-only, but stderr is empty on success)
// - Output filtering uses merged stdout+stderr as raw input (was stdout-only)
