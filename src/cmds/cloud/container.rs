...
fn docker_ps(_verbose: u8) -> Result<i32> {
    ...
    if !result.success() {
        ...
        timer.finish("docker ps", "rtk docker ps", &raw, &raw);
    ...
    if stdout.trim().is_empty() {
        ...
        println!("{}", rtk);
        timer.finish("docker ps", "rtk docker ps", &raw, &rtk);
        return Ok(0);
    }
...
fn docker_images(_verbose: u8) -> Result<i32> {
    ...
    if !result.success() {
        eprint!("{}", result.stderr);
        timer.finish("docker images", "rtk docker images", &raw, &raw);
        return Ok(result.exit_code);
    }
    ...
    if lines.is_empty() {
        ...
        println!("{}", rtk);
        timer.finish("docker images", "rtk docker images", &raw, &rtk);
        return Ok(0);
    }
...
pub fn run_compose_ps(verbose: u8) -> Result<i32> {
    ...
    println!("{}", rtk);
    timer.finish("docker compose ps", "rtk docker compose ps", &raw, &rtk);
    Ok(0)
}
