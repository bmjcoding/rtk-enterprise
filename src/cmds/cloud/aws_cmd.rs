...
/// When `truncated` is true, the filtered text notes the compacted output.
struct FilterResult {
    text: String,
    truncated: bool,
}
...
fn run_generic(subcommand: &str, args: &[String], verbose: u8, full_sub: &str) -> Result<i32> {
    let timer = execution::CommandExecution::start();
...
/// Follows the command wrapper contract: timer, execute, filter, no-op persistence hook, exit code.
fn run_aws_filtered(
    sub_args: &[&str],
    extra_args: &[String],
    verbose: u8,
    ...
    // Combine stdout+stderr for accurate output handling (per contract)
    ...
    if !status.success() {
        ...
        timer.finish(&cmd_label, &rtk_label, &raw, &stderr);
        return Ok(exit_code);
    }
    ...
    timer.finish(&cmd_label, &rtk_label, &raw, &result.text);
    Ok(0)
}
...
fn run_s3_ls(extra_args: &[String], verbose: u8) -> Result<i32> {
    ...
    if !output.status.success() {
        ...
        timer.finish("aws s3 ls", "rtk aws s3 ls", &raw, &stderr);
        return Ok(exit_code);
    }
    ...
    timer.finish("aws s3 ls", "rtk aws s3 ls", &raw, &result.text);
    Ok(0)
}
...
// FilterResult.truncated = true means items were cut from the compact display.
...
mod tests {
    ...
    fn test_filter_logs_events() {
        let json = r#"{
            ...
            "nextForwardToken": "f/EXAMPLE_FORWARD_TOKEN",
            "nextBackwardToken": "b/EXAMPLE_BACKWARD_TOKEN"
        }"#;
        ...
        assert!(!result.text.contains("EXAMPLE_FORWARD_TOKEN"));
        assert!(!result.truncated);
    }
    ...
    fn test_filter_logs_events_token_savings() {
        ...
        let json = format!(
            r#"{{"events": [{}], "nextForwardToken": "f/EXAMPLE_FORWARD_TOKEN_LONG", "nextBackwardToken": "b/EXAMPLE_BACKWARD_TOKEN_LONG"}}"#,
            events.join(",")
        );
        let result = filter_logs_events(&json).unwrap();
        let input_tokens = count_tokens(&json);
    ...
    fn test_filter_lambda_get() {
        let json = r#"{
            ...
            "Code": {"Location": "https://awslambda-us-east-1-tasks.s3.amazonaws.com/snapshots/123/my-func?versionId=example&X-Amz-Security-Token=EXAMPLE_REDACTED_TOKEN"},
            "Tags": {"Team": "backend"}
        }"#;
