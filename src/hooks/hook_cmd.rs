...
fn handle_vscode(cmd: &str) -> Result<()> {
    ...
    if verdict == PermissionVerdict::Deny {
        observe_hook_decision_noop("deny", cmd, "");
        return Ok(());
    }
    ...
    observe_hook_decision_noop("rewrite", cmd, &rewritten);
...
// ── Hook event persistence disabled for enterprise builds ─────

fn observe_hook_decision_noop(_action: &str, _original: &str, _rewritten: &str) {}
...
pub fn run_claude() -> Result<()> {
    ...
    match process_claude_payload(&v) {
        ...
        PayloadAction::Skip { reason, cmd } => {
            observe_hook_decision_noop(reason, &cmd, "");
        }
