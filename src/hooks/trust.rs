...
//! - `rtk trust` stores the reviewed SHA-256 hash
...
pub fn run_trust(list: bool) -> Result<()> {
    if list {
        ...
        println!("Trusted project filter keys:");
        println!("{}", "═".repeat(60));
        for (key, entry) in &trusted {
            println!("  filter-key:{}", key);
...
pub fn run_untrust() -> Result<()> {
    let filter_path = Path::new(".rtk/filters.toml");
    // If file doesn't exist, untrust by hashed filter key lookup won't work.
...
mod tests {
    ...
    fn trust_with_store(filter_path: &Path, store_file: &Path) -> Result<()> {
        ...
        store.version = 1;
        store.trusted.insert(key, TrustEntry { sha256: hash });
