...
If a command isn't in the list above, RTK runs it through passthrough — the output reaches the LLM unchanged. You can explicitly run unsupported commands without filtering:

```bash
rtk proxy make install    # runs make install with no filtering
```
