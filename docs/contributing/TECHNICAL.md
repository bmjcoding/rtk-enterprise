...
1. Start a no-op execution guard (`CommandExecution::start()`)
...
1. Guard: check if the command is an RTK meta-command (`init`, `config`, etc.) -- if so, show Clap error
2. Look up TOML DSL filters via `toml_filter::find_matching_filter()`
3. If TOML match: capture stdout and apply filter pipeline
4. If no match: pure passthrough with `Stdio::inherit`
...
Command received
  -> Clap parse succeeds?
     ...
     -> No:  run_fallback()
              -> TOML filter match?
                 -> Yes: Capture stdout, apply filter
                 -> No:  Passthrough (inherit stdio)
```
...
| [`core/`](../src/core/README.md) | Shared infrastructure | Config system, TOML filter engine, utility functions |
...
| [`discover/`](../src/discover/README.md) | Rewrite registry | Rewrite patterns and compound command splitting |
