...
6. [Global Flags Architecture](#global-flags-architecture)
7. [Error Handling](#error-handling)
8. [Configuration System](#configuration-system)
9. [Common Patterns](#common-patterns)
10. [Build Optimizations](#build-optimizations)
...
- **Core Infrastructure**: `src/core/` — utils, filter, config, toml_filter
- **Hook System**: `src/hooks/` — init, rewrite, permissions, hook_cmd, hook_check, verify, trust, integrity

### Module Count Breakdown

- **Command Modules**: 42 (directly exposed to users)
- **Infrastructure Modules**: shared runtime, configuration, hook, and filtering utilities
...
pytest_cmd.rs     STATE MACHINE         Text parser       90%+
  ...
  State transitions: IDLE -> TEST_START -> PASSED/FAILED -> SUMMARY
  Extract:
...
└─ Plain text only?
   ...
   │    Examples: pytest (test lifecycle parsing)
   │
...
Overhead Sources:
  ...
  • State machine: 3-8ms (regex + state transitions)
...
> For config file format, tee settings, TOML filter tiers, see [src/core/README.md](src/core/README.md).
