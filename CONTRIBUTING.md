...
**Scope** should match the module or area: `git`, `cargo`, `gh`, `hook`, `config`, `cicd`, etc.
...
Combine the prefix with a scope if it adds clarity (e.g. `git`, `kubectl`, `filter`, `config`) and finish with a descriptive slug: `fix/<scope>-<description>` or `feat/<description>`.
