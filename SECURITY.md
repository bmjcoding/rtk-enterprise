...
- **Data leaks** (unexpected persistence or network exfiltration)
...
Document exploitation examples in private review material rather than public issues.
...
Enterprise builds must pass the local release gate before packaging:
...
The enterprise gate enforces:

- No telemetry/reporting command surfaces (`gain`, `discover`, `learn`, `session`, `telemetry`, `hook-audit`, `cc-economics`)
...
// Hardcoded secrets
const API_KEY: &str = "<redacted example token>";
```
...
- **Security issues**: use GitHub private security advisories
- **General questions**: https://github.com/bmjcoding/rtk-enterprise/discussions
