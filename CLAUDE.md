...
- This project: RTK Enterprise (bmjcoding/rtk-enterprise)
- reachingforthejack/rtk: Rust Type Kit (DIFFERENT - generates Rust types)

**Verify correct installation:**
```bash
rtk --version  # Should show "rtk 0.28.2" (or newer)
rtk --help     # Should show RTK command filters
```

If the command set does not match this repository, you may have the wrong package installed.
...
rtk uses a **command proxy architecture**: `main.rs` routes CLI commands via a Clap `Commands` enum to specialized filter modules in `src/cmds/*/`, each of which executes the underlying command and compresses its output. This enterprise build does not store command usage data or token savings.
...
**Purpose**: Execute commands without filtering.
