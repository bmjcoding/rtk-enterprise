...
```bash
rtk init              # Configure hooks
rtk verify            # Verify installed hook files
...
> Name collision: unexpected `rtk` behavior can mean you have another binary installed. Run `which rtk` to check.
...
rtk routes CLI commands via a Clap `Commands` enum in `main.rs` to specialized filter modules in `src/cmds/*/`, each executing the underlying command and compressing output. Enterprise builds do not persist command history or emit external reports.
