...
This directory contains user-facing documentation for RTK Enterprise.
The enterprise fork keeps documentation local to the repository unless an internal documentation pipeline is configured.
...
- **No `rtk <cmd>` syntax**: users usually do not type `rtk` — hooks rewrite commands transparently.
  Only administrative commands such as `rtk init`, `rtk verify`, and `rtk proxy` appear as user-typed commands.
