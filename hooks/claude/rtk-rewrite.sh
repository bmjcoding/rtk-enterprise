...
if ! command -v rtk &>/dev/null; then
  echo "[rtk] WARNING: rtk is not installed or not in PATH. Hook cannot rewrite commands. Install: https://github.com/bmjcoding/rtk-enterprise#installation" >&2
  exit 0
fi
...
if [ ! -f "$CACHE_FILE" ]; then
  ...
  if [ -n "$RTK_VERSION" ]; then
    ...
    if [ "$MAJOR" -eq 0 ] && [ "$MINOR" -lt 23 ]; then
      echo "[rtk] WARNING: rtk $RTK_VERSION is too old (need >= 0.23.0). Upgrade from the approved RTK Enterprise source or package." >&2
      exit 0
    fi
  fi
